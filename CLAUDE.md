...
`AGENTS.md` is the canonical agent guide and is kept in step with this file. If they ever disagree, `AGENTS.md` wins.
...
4. `plan/UI_MIGRATION.md` for any UI work.
...
- Use `--!strict` in every Luau source file. The one approved exception is the React UI kit under `src/shared/UI`, which is `--!nonstrict` because React prop tables are open; its `State/` modules remain strict. See `plan/UI_MIGRATION.md`.
